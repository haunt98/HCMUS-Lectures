...
void radixSortLectK(int *arr, int size, int k) // count sort cho chu so thu k
{
    ...
    for (int i = 0; i < size; ++i)
    {
        ++count[digit(arr[i], k)]; // so lan xuat hien chu so thu k cua tung so
    }
    for (int i = 1; i < 10; ++i)
    {
...
void radixSortDA(int *arr, int size)
{
    ...
    int **count;
    for (int d = 1; d <= max_digit; ++d) // vong lap het so cac chu so cua max
    {
...
void radixSortMSDforK(int *arr, int low, int high, int k)
{
    if (low < high && k > 0)
    {
        ...
        for (int i = 1; i < 9; ++i)
            radixSortMSDforK(arr, low + count[i], low + count[i + 1] - 1,
                             k - 1);
