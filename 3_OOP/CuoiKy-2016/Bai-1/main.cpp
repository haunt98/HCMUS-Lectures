...
public:
    ...
    const type_info &InterfaceType() { return typeid(Shape); }
    const type_info &ImplementationType() { return typeid(*this); }
    virtual ~Shape() { cerr << "destruct Shape" << endl; }
};
...
public:
    Circle(float r) : Shape(), radius(r) { cerr << "construct Circle" << endl; }
    string Description()
    {
        stringstream ss;
    ...
    void Scale(float scaleFactor) { this->radius *= scaleFactor; }
    const type_info &InterfaceType() { return typeid(Circle); }
...
public:
    ...
    const type_info &InterfaceType() { return typeid(Ellipse); }
    ~Ellipse() { cerr << "destruct Ellipse" << endl; }
};
