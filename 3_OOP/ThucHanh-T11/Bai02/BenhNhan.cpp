...
void BN_NoiTru::updateMoney(int dayOut)
{
    if (m_dayOut == 0)
    {
        ...
        if (m_phong == 'A')
        {
            m_money = (m_dayOut - m_dayIn) * (m_moneyPerDay + PHONG_A);
        }
        else if (m_phong == 'B')
        {
            m_money = (m_dayOut - m_dayIn) * (m_moneyPerDay + PHONG_B);
        }
        else if (m_phong == 'C')
        {
            m_money = (m_dayOut - m_dayIn) * (m_moneyPerDay + PHONG_C);
        }
    }
}
...
void BN_NoiTru::writehistory(float moneyPerDay)
{
    ...
    if (m_dayOut == 0)
    {
        file << m_dayIn << " NV " << moneyPerDay << " " << m_phong << endl;
    }
    else
    {
