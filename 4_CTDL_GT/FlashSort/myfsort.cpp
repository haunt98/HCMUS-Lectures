...
void myFlashSort(int *arr, int size)
{
    ...
    while (nMove < size - 1) // Khi di chuy?n size - 1 bu?c thì d?ng
    {
        ...
        {
            ++j;
            K = c * (arr[j] - min) + 1; // C?p nh?p l?i v? trí biên c?a arr[j]
        }
        int flash = arr[j]; // flash luu ph?n t? c?n di chuy?n vì sai v? trí
        while (j < L[K])    // d?ng l?i khi h?t m?t l?p
        {
            K = c * (flash - min) + 1; // C?p nh?p l?i v? trí biên c?a flash
            --L[K];                    // Gi?m v? trí biên
            swap(flash, arr[L[K]]);    // Ðua flash vào v? trí dúng c?a nó, d?ng
                                       // th?i copy ph?n t? cu vào flash
            ++nMove;
        }
    }
    delete[] L;
