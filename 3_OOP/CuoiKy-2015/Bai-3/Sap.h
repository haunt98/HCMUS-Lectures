...
class SAP
{
	...
	SAP()
	{
		mThuTu = mDienTich = mDoanhThu = mThue = 0;
	}
	...
	float tienThueSap() const
	{
		return DON_GIA * mDienTich;
	}
	float tienCuoiNam() const
	{
		return tienThueSap() + mThue * mDoanhThu;
	}
	void updateThue(float Thue)
	{
		mThue = Thue;
	}
