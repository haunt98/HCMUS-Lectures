...
template <class T>
void sortArr(T *arr, int size, bool(*Compare)(T, T))
{
...
template <class T>
bool tangArr(T x, T y)
{
	return x < y;
}
template <class T>
bool giamArr(T x, T y)
{
	return !tangArr(x, y);
}
template <class T>
...
template <class T>
void addHead(Node<T> *&head, T key)
{
	Node<T> *p = new Node<T>;
	if (p != NULL)
	{
		p->key = key; p->next = NULL;
		if (head == NULL)
			head = p;
		else
		{
...
template<class T>
bool tangList(Node<T> *x, Node<T> *y)
{
	return x->key < y->key;
}
template<class T>
bool giamList(Node<T> *x, Node<T> *y)
{
	return !tangList(x, y);
}
template <class T>
...
int main()
{
	...
	PHANSO arr5[] = { { 5, 4 },{ 4, 3 },{ 3, 2 },{ 2, 1 },{ 6, 5 },{ 7, 6 },{ 8, 7 },{ 9, 8 } };
