...
public:
    ...
    template <class F>
    friend istream &operator>>(istream &in, Mang<F> &mg);
    template <class F>
...
// gan =
template <class T>
Mang<T> &Mang<T>::operator=(const Mang<T> &mg)
{
    m_size = mg.m_size;
    if (m_arr != NULL)
        delete[] m_arr;
...
// nhap xuat
template <class T>
istream &operator>>(istream &in, Mang<T> &mg)
{
    in >> mg.m_size;
    if (mg.m_arr == NULL)
    {
...
template <class T>
ostream &operator<<(ostream &out, const Mang<T> &mg)
{
    out << "[" << mg.m_size << "] ";
    for (int i = 0; i < mg.m_size; i++)
    {
