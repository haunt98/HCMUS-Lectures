...
{
	int i;
	printf("Cac vi tri co gia tri la cac so nguyen co chu so dau tien la "
	       "so le: ");
	for (i = 0; i < len_mang; ++i) {
		if (dau_le(arr[i]))
			printf("%d, ", i + 1);
...
void lke_nhieu_nhat(int arr[], int len_mang) // Bai 238
{
	...
	for (i = 1; i < n_len; ++i) {
		if (tan_suat(mang_tam[i], arr, len_mang) >
		    max) // Tim so lan xuat hien nhieu nhat
...
int dem_nguyento_phanbiet(int arr[], int len_mang) // Bai 239
{
	...
	n_len = 0;
	for (i = 0; i < len_mang; ++i) {
		if (tan_suat(arr[i], mang_tam, n_len) ==
		    0) // Xoa trung lap trong mang
			mang_tam[n_len++] = arr[i];
	}
	for (i = 0; i < n_len; ++i) {
...
void sx_chan_le(int arr[], int len_mang) // Bai 262
{
	...
	sap_xep_tang(mang_chan, len_chan);
	sap_xep_tang(mang_le, len_le);
