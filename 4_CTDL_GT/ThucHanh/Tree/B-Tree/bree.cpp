...
void B_Tree_Split_Child(pNode &father, int i)
{
	...
	if (!old_son->leaf) {
		...
		new_son->children =
		    (pNode *)malloc(sizeof(pNode) * (new_son->n + 1));
		for (int j = 0; j < T_MIN_DEGREE; ++j) {
			// new_son->c[0..t-1] and old_son->c[t..2t-1]
			new_son->children[j] =
			    old_son->children[j + T_MIN_DEGREE];
		}
	}
	...
	father->key =
	    (int *)realloc(father->key, sizeof(int) * (father->n + 1));
	...
	if (!old_son->leaf) {
		old_son->children = (pNode *)realloc(
		    old_son->children, sizeof(pNode) * (old_son->n + 1));
	}
}
...
void B_Tree_Insert_NonFull(pNode &x, int k)
{
	...
	if (x->leaf) {
		...
		if (x->children[i]->n == 2 * T_MIN_DEGREE - 1) {
			...
			// x now have 2 children, which child we should insert
			// k? key:          i children:    i i+1
			if (x->key[i] < k) {
				++i;
			}
		}
