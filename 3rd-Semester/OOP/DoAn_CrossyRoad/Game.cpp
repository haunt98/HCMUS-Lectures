...
void Game::CreObj(int level)
{
    ...
    for (int i = 0; i < LINE_DINA; ++i)
    {
        ...
        {
            m_klong[i][j] = Dina(ToaDo(1 + j * WIDTH_DINA, 2 + i * HEIGHT_OBJ));
        }
    }
    ...
    for (int i = 0; i < LINE_BIRD; ++i)
    {
        ...
        {
            m_chim[i][j] = Bird(ToaDo(1 + j * WIDTH_BIRD, 6 + i * HEIGHT_OBJ));
        }
    }
    ...
    for (int i = 0; i < LINE_TRUCK; ++i)
    {
        ...
        {
            m_xetai[i][j] = Truck(
                ToaDo(WIDTH_BOARD - 2 - j * WIDTH_TRUCK, 9 + i * HEIGHT_OBJ));
        }
    }
    ...
    for (int i = 0; i < LINE_CAR; ++i)
    {
        ...
        {
            m_xehoi[i][j] = Car(
                ToaDo(WIDTH_BOARD - 2 - j * WIDTH_CAR, 12 + i * HEIGHT_OBJ));
        }
    }
}
...
void Game::Save(const string &saveFile)
{
    ...
    outF << m_level << endl;
    // Dina
    outF << COL_DINA << " " << COL_BIRD << " " << COL_TRUCK << " " << COL_CAR
         << endl;
    if (m_klong)
    {
        for (int i = 0; i < LINE_DINA; ++i)
