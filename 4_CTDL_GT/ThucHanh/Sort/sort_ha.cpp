...
void radixSort(int *arr, int size)
{
    ...
    clock_t end = clock();
    cout << "Radix sort: " << (double)(end - begin) / CLOCKS_PER_SEC << endl;
}
...
void countSortMod(int *arr, int size, int d)
{
    ...
    {
        count[i] += count[i - 1]; // count[i] the hien so cac chu so <= i
    }
...
void flashSort(int *arr, int size)
{
    ...
    clock_t end = clock();
    cout << "Flash sort: " << (double)(end - begin) / CLOCKS_PER_SEC << endl;
}
