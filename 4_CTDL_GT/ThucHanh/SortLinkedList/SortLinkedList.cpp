    ...
    while (key)
    {
             ...
             prevP = p, p = p->pNext)
        {
            if (p->key > key->key)
            {
                ...
                // dong thoi thay doi prevKey->pNext de tiep tuc vong lap key
                prevP->pNext = key;
...
void selectionSort(List &l)
{
    ...
    pNode prevList = getNode(-1);
    prevList->pNext = l.pHead;
    for (pNode prevP = prevList, p = prevP->pNext; p; prevP = p, p = p->pNext)
    {
        // q chay sau p
        for (pNode prevQ = p, q = p->pNext; q; prevQ = q, q = q->pNext)
        {
            if (p->key > q->key)
            {
                if (prevQ == p)
