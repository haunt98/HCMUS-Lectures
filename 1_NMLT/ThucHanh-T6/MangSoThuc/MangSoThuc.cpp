...
{
	printf("Cac gia tri nho hon tri tuyet doi gia tri lien sau va lon hon "
	       "gia tri lien truoc: ");
	int i;
...
void lke_duong_nho_nhat(float arr[], int len_mang) // Bai 187
{
	...
	printf("Vi tri ma gia tri tai do bang gia tri duong nho nhat trong "
	       "mang: ");
	if (min == -1)
		return;
	else {
		...
		for (j = i + 1; j < len_mang - 1; ++j) {
			for (k = j + 1; k < len_mang; ++k) {
				if (arr[i] == arr[j] + arr[k])
					printf("(%0.2f,%0.2f,%0.2f) ", arr[i],
					       arr[j], arr[k]);
				if (arr[j] == arr[k] + arr[i])
					printf("(%0.2f,%0.2f,%0.2f) ", arr[j],
					       arr[k], arr[i]);
				if (arr[k] == arr[i] + arr[j])
					printf("(%0.2f,%0.2f,%0.2f) ", arr[k],
					       arr[i], arr[j]);
			}
		}
	}
...
void lke_tan_suat(float arr[], int len_mang) // Bai 233
{
	...
	for (i = 0; i < n_len; ++i) {
		printf("%0.2f           %d\n", mang_tam[i],
		       tan_suat(mang_tam[i], arr, len_mang));
	}
}
			  ...
			  int len_2) // Bai 234
{
	...
	for (i = 0; i < len_1; ++i) {
		if (tan_suat(arr1[i], mang_tam_1, n_len_1) ==
		    0) // Loai trung lap mang 1
			mang_tam_1[n_len_1++] = arr1[i];
	}
	for (i = 0; i < n_len_1; ++i) {
		if (tan_suat(mang_tam_1[i], arr2, len_2) ==
		    0) // Xuat hien mang 1 nhung khong co trong mang 2
			++dem;
	}

	for (i = 0; i < len_2; ++i) {
		if (tan_suat(arr2[i], mang_tam_2, n_len_2) ==
		    0) // Loai trung lap mang 2
			mang_tam_2[n_len_2++] = arr2[i];
	}
	for (i = 0; i < n_len_2; ++i) {
		if (tan_suat(mang_tam_2[i], arr1, len_1) ==
		    0) // Xuat hien mang 2 nhung khong co trong mang 1
			++dem;
	}
	return dem;
}
...
int ktra_a_trong_b(float arr1[], int len_1, float arr2[], int len_2) // Bai 253
{
	...
	if (len_1 > len_2)
		...
		for (i = 0; i < len_1; ++i) {
			if (tan_suat(arr1[i], arr2, len_2) <
			    tan_suat(arr1[i], arr1, len_1))
				flag = 0;
		}
	}
	return flag;
...
int ktra_hoanvi(float arr1[], int len1, float arr2[], int len2) // Bai 260
{
	...
	if (len1 != len2)
		...
		for (i = 0; i < len1; ++i)
			if (tan_suat(arr1[i], arr1, len1) !=
			    tan_suat(arr1[i], arr2, len2))
				flag = 0;
	}
	return flag;
}
