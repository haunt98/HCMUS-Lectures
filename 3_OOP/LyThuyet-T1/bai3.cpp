...
template <class T> bool beHon(T a, T b)
{
	return a < b;
}

bool beHon(PhanSo a, PhanSo b)
{
	return a.tu * b.mau < b.tu * a.mau;
}

template <class T> bool lonHon(T a, T b)
{
	return a > b;
}
...
template <class T> void sxepMang(T *arr, int size, bool (*dieuKien)(T, T))
{
	for (int i = 0; i < size; i++) {
		for (int j = i + 1; j < size; j++) {
			if (!dieuKien(arr[i],
				      arr[j])) // khong dung dieu kien thi swap
			{
				swap(&arr[i], &arr[j]);
			}
		}
...
template <class T> void printType(T a)
{
	cout << a << " ";
}

void printType(PhanSo a)
{
	printf("%d/%d ", a.tu, a.mau);
}

template <class T> void inMang(T *arr, int size)
{
	for (int i = 0; i < size; i++) {
		printType(arr[i]);
	}
